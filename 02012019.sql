...
BEGIN
 RETURN QUERY select "Post".id, "Post".id_user, "User".name, "User".surname, "User".username, "User".avatar_src, "Post".content, "Post".date, COUNT("Like".id), COUNT("Comment".id), CASE 
            	...
            	WHEN "Comment".id_user = _logined_user
       			...
       			END as my_comment
			  FROM "Post"
			  left join "Subscribe" on ("Post".id_user = "Subscribe".id_user_subscribe and "Subscribe".id_user = _logined_user)
			  join "User" on ("User".id = _logined_user and "Post".id_user = _logined_user)
...
CREATE FUNCTION public.get_all_user_posts(_current_user character varying, _logined_user integer) RETURNS TABLE(id integer, content text, date timestamp without time zone, likes bigint, comments bigint, my_like boolean, my_comment boolean)
    LANGUAGE plpgsql
    AS $$
BEGIN
 RETURN QUERY SELECT "Post".id, "Post".content, "Post".date, COUNT("Like".id), COUNT("Comment".id), CASE 
...
SELECT pg_catalog.setval('public."Like_id_seq"', 44, true);
...
SELECT pg_catalog.setval('public."Post_id_seq"', 34, true);
...
SELECT pg_catalog.setval('public."Subscribe_id_seq"', 45, true);
